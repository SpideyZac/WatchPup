...
export async function doesUserOwnRequest(user: User, request: RecordId) {
    const requested_service = await queryOne<RequestedService>("SELECT * FROM requested_service WHERE in = $request", {
        request,
    });
    ...
    return services.find((service) => service.id.id === requested_service[0].out.id);
}